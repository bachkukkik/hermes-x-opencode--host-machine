...
#
# NOTE: All credentials are inlined into the generated config files
# (opencode.jsonc, auth.json, config.yaml) at generation time. opencode and
# hermes therefore run from ANY directory with NO environment variables set,
# so there is no shell/rc-file integration and nothing leaks machine-wide.
#
...
# dependency and no machine-wide env bridge that would leak secrets.
export OPENAI_API_KEY OPENAI_BASE_URL OPENCODE_ZEN_API_KEY
...
# TC2a: opencode.jsonc provider.opencode has a non-empty Zen apiKey.
# The key is inlined at generation time when OPENCODE_ZEN_API_KEY is set,
# else it falls back to the {env:OPENCODE_ZEN_API_KEY} placeholder — both are
# valid, non-empty apiKey values. Read the actual staging value as the source
# of truth rather than asserting a specific literal.
_zen_apikey=$(python3 -c "import json,sys; print(json.load(open(sys.argv[1])).get('provider',{}).get('opencode',{}).get('options',{}).get('apiKey',''))" "${STAGING_OPENCODE}" 2>/dev/null || echo "")
if [ -n "$_zen_apikey" ]; then
    _pass "opencode.jsonc provider.opencode has apiKey (non-empty)"
else
    _fail "opencode.jsonc provider.opencode apiKey missing or empty"
fi
...
if [ "$APPLY" = true ]; then
    ...
    _staging_sources=("${STAGING_OPENCODE}" "${STAGING_HERMES_OVERLAY}" "${STAGING_AUTH}" "${STAGING_DCP}")
    _live_dests=("${OPENCODE_CONFIG}" "${CONFIG}" "${OPENCODE_AUTH}" "${OPENCODE_DCP_CONFIG}")
    _apply_labels=("OpenCode config" "Hermes overlay" "OpenCode auth" "DCP config")
