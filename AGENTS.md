...
- **Free Zen model requirement:** OpenCode must default to `opencode/deepseek-v4-flash-free` so that Hermes→OpenCode delegation burns no paid token quota. The credential `OPENCODE_API_KEY` is read from `~/.hermes/.env` and used directly as the `{env:OPENCODE_API_KEY}` value in the opencode provider block.
