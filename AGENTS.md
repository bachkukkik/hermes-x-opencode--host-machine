...
- **Free Zen model requirement:** OpenCode must default to `opencode/deepseek-v4-flash-free` so that Hermes→OpenCode delegation burns no paid token quota. The credential `OPENCODE_ZEN_API_KEY` is read from `~/.hermes/.env` and mapped to `{env:OPENCODE_API_KEY}` in the opencode provider block.
